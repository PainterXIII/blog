...
HTML;
    ...
    public static function getCdnType($host) {
        ...
        // check if local
        $local = parse_url(self::$themeOptions->index);
    ...
    private static function getUploadsFilePath($url) {
        if (!Mirages_Utils::startsWith($url, self::$themeOptions->index)) {
            return false;
        }
        $url = rtrim(preg_replace('/^' . preg_quote(rtrim(self::$themeOptions->index, '/'), '/') . '/', '', $url, 1), '/');
